...
with tf.Session() as sess:
     patches =  tf.extract_image_patches(images=img, ksizes=[1, 16, 16, 1], strides=[1, 1, 1, 1], 
                                         rates=[1, 1, 1, 1], padding='SAME')
     patches = tf.reshape(patches,[-1,16,16,3])
     
     patches2 =  tf.extract_image_patches(images=img2, ksizes=[1, 16, 16, 1], strides=[1, 1, 1, 1], 
                                         rates=[1, 1, 1, 1], padding='SAME')
...
x_train = fdata[:,0:768].reshape(fdata.shape[0], patch_width, patch_height, 3)
...
#%%
with tf.Session() as sess:
    patches3 =  tf.extract_image_patches(images=img, ksizes=[1, 16,16, 1], strides=[1, 1, 1, 1], 
                                         rates=[1, 1, 1, 1], padding='SAME')
    patches3 = tf.reshape(patches3,[-1,16,16,3])
...
while(y<mask.shape[0]):
    ...
    y += 1
    if (y % 32 == 0):
        print("No. of iterations till results = {}".format(256-y))
