...
ndimn = (128,128)
#%%
def rotate(arr,dimns):
    smallarr = misc.imresize(arr,dimns)
...
def img_preprocess(qa,b4,b3,b2, tdimn):    
    ...
    angle = rotate(mask,ndimn)
    print (angle)
    mask = misc.imrotate(mask,angle,interp='nearest')
    ...
    orig = misc.imrotate(orig,angle,interp='nearest')
...
#%%patch extraction
def train_data(imin,imax):
    cloud_t = []
    bk_t = []
    for i in range(imin,imax): 
        qa,b4,b3,b2 = df['BQA'].iloc[i],df['B4'].iloc[i],df['B3'].iloc[i],df['B2'].iloc[i]
        get_imgs = img_preprocess(qa,b4,b3,b2,tdimn)
        img, img2 = get_imgs[0], get_imgs[1]
...
#%%train_data
fdata = train_data(0,6)
vdata = train_data(6,9)
tdata = train_data(9,11)
